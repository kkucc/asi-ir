...
PULSE_DURATION_MS = 10 # Длительность TTL импульса в мс
...
class MS2000Controller:
    ...
    def wait_for_idle(self):
        ...
        while not self.stop_event.is_set():
            # Убираем логирование из цикла, чтобы не было спама
            if self.ser and self.ser.write(b'/\r') and self.ser.read(1) == b'N': return
    ...
    def run_scan(self, params, device: AcquisitionDevice, line_callback):
        ...
        try:
            # НАСТРОЙКА TTL ПЕРЕД НАЧАЛОМ СКАНА 
            self.log(f"INFO: Setting TTL pulse duration to {PULSE_DURATION_MS} ms")
            self.send_command(f"RT Y={PULSE_DURATION_MS}")
            self.log("INFO: Enabling 'Pulse on Move Completion' mode")
            self.send_command("TTL Y=2")
            ...
            for i, y in enumerate(y_coords):
                ...
                for j, x in enumerate(xs):
                    ...
                    # Сразу после wait_for_idle контроллер сам отправил импульс.
                    # Теперь мы собираем данные.
            ...
            #  ОТКЛЮЧАЕМ TTL ПОСЛЕ СКАНА 
            self.log("INFO: Disabling TTL pulse mode.")
            self.send_command("TTL Y=0") # Возвращаем выход в обычное состояние
...
class StageControlApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("MS-2000 Cockpit v15 (TTL Ready)"); self.root.geometry("700x520")

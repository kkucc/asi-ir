...
class MS2000Controller:
    ...
    def _set_high_precision(self):
        ...
        except Exception: pass #  не критично
...
class StageControlApp:
    ...
    def _create_widgets(self):
        ...
        main_frame.rowconfigure(1, weight=1)    # Строка с параметрами расширяется

        # Создание фреймов по сетке
        ...
        top_right_frame.grid_propagate(False) # Запрещаем сжиматься/расширяться
        ...
        self.conn_entries={}
        for i,(t,v) in enumerate({"Port":"COM4","Baudrate":"9600"}.items()):ttk.Label(conn_frame,text=f"{t}:").grid(row=i,column=0,sticky="w");e=ttk.Entry(conn_frame,width=8);e.insert(0,v);e.grid(row=i,column=1);self.conn_entries[t.lower()]=e
        self.connect_button=ttk.Button(conn_frame,text="Connect",command=self.connect);self.connect_button.grid(row=0,column=2,rowspan=2,padx=5)
        ...
        # 2. Top Right: Minimap and its controls
        self.fig = Figure(figsize=(2.5, 2.5), dpi=100)
        self.ax = self.fig.add_subplot(111)
        self.fig.subplots_adjust(left=0, right=1, top=1, bottom=0) # Убираем поля
        self.canvas = FigureCanvasTkAgg(self.fig, master=top_right_frame)
        self.canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
        self.setup_minimap()

        map_controls_frame = ttk.Frame(top_right_frame)
        map_controls_frame.pack(fill=tk.X, pady=5)
        ...
        self.scan_entries={}
        scan_fields = {"start_x":("Start X (mm)","45.0"),"steps_x":("Points X","50"),"step_x":("Step X (mm)","0.2"),"start_y":("Start Y (mm)","45.0"),"steps_y":("Points Y","50"),"step_y":("Step Y (mm)","0.2"),"speed":("Speed (mm/s)","2.0"),"dwell":("Dwell (s)","0.01")}
        for i,(k,(l,v)) in enumerate(scan_fields.items()):
            row, col = divmod(i, 4)
            ttk.Label(param_frame, text=l+":").grid(row=row, column=col*2, sticky="w",pady=2);e=ttk.Entry(param_frame,width=8);e.insert(0,v);e.grid(row=row,column=col*2+1,padx=5);e.bind("<KeyRelease>",self.update_scan_area_preview);self.scan_entries[k]=e

    # УПРАВЛЕНИЯ МИНИКАРТОЙ
    def setup_minimap(self):
        self.ax.clear(); self.ax.set_xticks([]); self.ax.set_yticks([]);
        self.ax.set_facecolor('#cccccc'); self.ax.set_aspect('equal', adjustable='box')
    ...
    def update_minimap_view(self):
        self.ax.set_xlim(self.minimap_extents[0], self.minimap_extents[1])
        self.ax.set_ylim(self.minimap_extents[2], self.minimap_extents[3])
        self.canvas.draw()

    def zoom_in(self):
        x0,x1,y0,y1 = self.minimap_extents; cx,cy=(x0+x1)/2,(y0+y1)/2; w,h=(x1-x0)/2,(y1-y0)/2
        self.minimap_extents = [cx-w/2, cx+w/2, cy-h/2, cy+h/2]
        self.update_minimap_view()

    def zoom_out(self):
        x0,x1,y0,y1 = self.minimap_extents; cx,cy=(x0+x1)/2,(y0+y1)/2; w,h=(x1-x0)*2,(y1-y0)*2
        self.minimap_extents = [max(0,cx-w/2), min(FIELD_OF_VIEW_SIZE,cx+w/2), max(0,cy-h/2), min(FIELD_OF_VIEW_SIZE,cy+h/2)]
        self.update_minimap_view()
    ...
    def update_scan_area_preview(self, event=None):
        ...
        try:
            ...
            width = (params['steps_x'] - 1) * params['step_x']
            height = (params['steps_y'] - 1) * params['step_y']
            self.scan_area_patch = self.ax.add_patch(tk.Rectangle((params['start_x'], params['start_y']), width, height, linewidth=1, edgecolor='black', facecolor='black', alpha=0.5))
            self.canvas.draw()
    ...
    def start_scan(self):
        ...
        self.scan_thread = threading.Thread(target=self.controller.run_scan, args=(params, self.line_update_callback), daemon=True)
        self.scan_thread.start()
        self.check_scan_thread()

    def line_update_callback(self, partial_data):
    ...
    def get_scan_params(self, validate_integrality=True):
        try:
            ...
            if validate_integrality and (params['steps_x'] % 1 != 0 or params['steps_y'] % 1 != 0 or params['steps_x'] < 1 or params['steps_y'] < 1):
                print("ERROR: Points must be integers >= 1."); return None
...
if __name__ == "__main__":
    from matplotlib.patches import Rectangle as tkRectangle
    root = tk.Tk()

...
class AcquisitionDevice:
    """Базовый класс-шаблон для любого измерительного устройства."""
...
class MS2000Controller:
    ...
    def run_scan(self, params, device: AcquisitionDevice, line_callback):
        ...
        try:
            self.send_command(f"RT Y={PULSE_DURATION_MS}"); self.send_command("TTL Y=2")
            ...
            self.log("INFO: Disabling TTL pulse mode."); self.send_command("TTL Y=0")
...
class StageControlApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("MS-2000 Cockpit v16 (NameError Fix)")

...
class SmartDummySignal(AcquisitionDevice):
    def acquire(self, dwell_time: float, x: float, y: float) -> float:
        ...
        if distance < radius:
            return 90 + np.random.rand() * 10 
        else:
            return 10 + np.random.rand() * 10
...
class MS2000Controller:
    ...
    def wait_for_idle(self):
        ...
        while not self.stop_event.is_set():
            if self.send_command("/", quiet=True) == 'N': 
                return
            if time.time() - start_time > timeout: 
                raise TimeoutError("Move command timed out")
            time.sleep(0.05)
    ...
    def move_relative(self, dx_gui, dy_gui):
        ...
        if self.swap_xy.get(): 
            dx_stage, dy_stage = dy_gui, dx_gui
        ...
        try:
            ...
            for i, y in enumerate(y_coords):
                if i > 0:
                    ...
                    self.move_absolute(x_coords[0], y - backlash_y)
                    self.wait_for_idle()
                    self.move_absolute(x_coords[0], y)
                    self.wait_for_idle()
                ...
                for j, x in enumerate(x_coords):
                    ...
                    if j > 0:
                        self.move_absolute(x + backlash_y, y)
                        self.wait_for_idle()
                        self.move_absolute(x, y)
                        self.wait_for_idle()

                    self.send_command("TTL Y=2", quiet=True)
                    
                    value = device.acquire(params['dwell'], x, y)
                    results[i, j] = value
                
                if line_callback: 
                    line_callback(results.copy(), i)
...
class StageControlApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("MS-2000 Cockpit v28 (Clean Formatted)")
    ...
    def _create_widgets(self):
        ...
        top_right = ttk.Frame(main_frame, width=280)
        bottom_left = ttk.Frame(main_frame)
        
        top_left.grid(row=0, column=0, rowspan=2, sticky="nsew", padx=(0, 10))
        ...
        e = ttk.Entry(conn_frame, width=8)
        e.insert(0, "COM4")
        e.grid(row=0, column=1)
        self.conn_entries['port'] = e
        
        ttk.Label(conn_frame, text="Baud:").grid(row=1, column=0, sticky="w")
        e = ttk.Entry(conn_frame, width=8)
        e.insert(0, "9600")
        e.grid(row=1, column=1)
        self.conn_entries['baudrate'] = e
        
        check_frame = ttk.Frame(conn_frame)
        check_frame.grid(row=0, column=2, rowspan=2, padx=10)
        ...
        btn_frame.grid(row=0, column=3, rowspan=2, padx=5)
        self.connect_button = ttk.Button(btn_frame, text="Connect", command=self.connect)
        ...
        self.device_combobox = ttk.Combobox(scan_ctrl_frame, values=[str(d) for d in self.available_devices], state="readonly")
        self.device_combobox.current(0)
        self.device_combobox.pack(fill=tk.X, pady=(0,5))
        
        self.start_scan_button = ttk.Button(scan_ctrl_frame, text="Start Scan", command=self.start_scan, state=tk.DISABLED)
        self.start_scan_button.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(0,5))
        ...
        self.canvas.get_tk_widget().pack(fill=tk.X, anchor='n')
        self.setup_minimap()
        
        map_controls = ttk.Frame(top_right)
        map_controls.pack(fill=tk.X, pady=5, anchor='n')
        ttk.Label(map_controls, text="Map View:").pack(side=tk.LEFT)
        ttk.Button(map_controls, text="+", width=3, command=self.zoom_in).pack(side=tk.LEFT)
        ttk.Button(map_controls, text="-", width=3, command=self.zoom_out).pack(side=tk.LEFT)
        ttk.Button(map_controls, text="Reset", command=self.reset_zoom).pack(side=tk.LEFT)
        ...
        btn_bar_frame = ttk.Frame(param_frame)
        btn_bar_frame.grid(row=0, column=0, columnspan=6, pady=(0, 5), sticky="ew")
        
        get_pos_button = ttk.Button(btn_bar_frame, text="Get Current as Start", command=self.get_current_position_as_start)
        get_pos_button.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0,5))
        
        set_center_button = ttk.Button(btn_bar_frame, text="Set Current as Center", command=self.set_current_position_as_center)
        set_center_button.pack(side=tk.LEFT, fill=tk.X, expand=True)
        ...
        for i, (k, (l, v, u)) in enumerate(scan_fields_x.items()): 
            ...
            e.insert(0, v)
            e.grid(row=i+2, column=1, padx=5)
            e.bind("<KeyRelease>", self.update_scan_area_preview)
            self.scan_entries[k] = e
            ttk.Label(param_frame, text=u).grid(row=i+2, column=2, sticky='w')
            
        for i, (k, (l, v, u)) in enumerate(scan_fields_y.items()): 
            ttk.Label(param_frame, text=l+":").grid(row=i+2, column=3, sticky="w")
            e = ttk.Entry(param_frame, width=8)
            e.insert(0, v)
            e.grid(row=i+2, column=4, padx=5)
            e.bind("<KeyRelease>", self.update_scan_area_preview)
            self.scan_entries[k] = e
            ttk.Label(param_frame, text=u).grid(row=i+2, column=5, sticky='w')
            
        param_frame.columnconfigure(2, minsize=15)
        ...
        for i, (k, (l, v, u)) in enumerate(scan_fields_general.items()): 
            ...
            e = ttk.Entry(param_frame, width=8)
            e.insert(0, v)
            e.grid(row=7, column=i*3+1, padx=5)
            self.scan_entries[k] = e
    ...
    def _manual_move(self, dx_factor, dy_factor):
        ...
        try:
            ...
            dx = step_x * dx_factor
            dy = step_y * dy_factor
            threading.Thread(target=self.controller.move_relative, args=(dx, dy), daemon=True).start()
        except (ValueError, KeyError):
            messagebox.showerror("Error", "Invalid step value in Scan Parameters.")
    ...
    def set_current_position_as_center(self):
        ...
        if not params: 
            messagebox.showerror("Error", "Invalid scan size parameters.")
            return
        pos = self.controller.get_position()
        if pos:
    ...
    def update_minimap_view(self): 
        self.ax.set_xlim(self.minimap_extents[0:2])
        self.ax.set_ylim(self.minimap_extents[2:4])
        self.canvas.draw()
    ...
    def zoom_out(self): 
        ...
        cx, cy = (x0 + x1) / 2, (y0 + y1) / 2
        w, h = (x1 - x0), (y1 - y0)
        self.minimap_extents =[max(STAGE_X_MIN, cx - w), min(STAGE_X_MAX, cx + w), max(STAGE_Y_MIN, cy - h), min(STAGE_Y_MAX, cy + h)]
        self.update_minimap_view()
    ...
    def on_pan_motion(self, e):
        ...
        dx = e.xdata - self._pan_start_x
        dy = e.ydata - self._pan_start_y
        self.minimap_extents[0] -= dx
        self.minimap_extents[1] -= dx
        self.minimap_extents[2] -= dy
        self.minimap_extents[3] -= dy
        self.update_minimap_view()
    ...
    def start_scan(self):
        ...
        dev_str = self.device_combobox.get()
        ...
        self.plot_scan_data(nan_data, -1)
        self.scan_thread = threading.Thread(target=self.controller.run_scan, args=(params, device, self.line_update_callback), daemon=True)
        self.scan_thread.start()
        self.check_scan_thread()

    def line_update_callback(self, data, row_idx): 
        self.root.after(0, self.plot_scan_data, data, row_idx)
    ...
    def get_scan_params(self, validate=True):
        try:
            ...
            if validate and (p['steps_x'] % 1 != 0 or p['steps_y'] % 1 != 0 or p['steps_x'] < 1 or p['steps_y'] < 1):
                messagebox.showerror("Parameter Error", "Points must be integers >= 1.")
                return None
            p['end_x'] = p['start_x'] + (p['steps_x'] - 1) * p['step_x']
            p['end_y'] = p['start_y'] + (p['steps_y'] - 1) * p['step_y']
            return p
        except (ValueError, tk.TclError):
            if validate:
                messagebox.showerror("Parameter Error", "All fields must contain valid numbers.")
            return None
